...
export function createApiMock(partial: Partial<ApiMock> = {}): ApiMock {
  const api: ApiMock = {
    ...
    githubImport: {
      ...
      plan: vi.fn(async () => ({ id: 'plan-1', items: [], blockers: [], warnings: [] })),
      previewComponent: vi.fn(async () => ({ componentId: 'component', files: [], revision: { ref: 'main' } })),
      importComponents: vi.fn(async () => []),
      activateHook: vi.fn(async () => ({ success: true, activation: {} })),
      deactivateHook: vi.fn(async () => ({ success: true })),
      runFallback: vi.fn(async () => ({ success: true, stdout: '', stderr: '' })),
      onProgress: vi.fn(() => () => {}),
      onComponentProgress: vi.fn(() => () => {}),
